...
test('Pages production assets and navigation stay under the project path', async ({ page, request }) => {
  const errors: string[] = [];
  ...
  for (const asset of assets) {
    expect(asset).toMatch(/^\/verse-recall\/assets\/[^/]+\.(js|css)$/);
  ...
  expect(new URL(page.url()).pathname).toBe('/verse-recall/');
